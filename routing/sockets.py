...
def connectToNotif():
    if 'id' in session:
        ...
        toSend.reverse()
        for notif in toSend:
            html = render_template("notification.html", notif=notif, similar=len(notif.getSimilar(session['id'])))
            ...
            if grp != None:
                if session['id'] in grp['id_utilisateurs']: # authorized
                    ...
                    if not json['contenuMessage'] == '':
                        ...
                        message = Message(id_groupe=json['room'], id_utilisateur=session['id'], contenu=contenu, reponse=reponse)
                        message.insert()
                    ...
                    if message:
                        ...
                        Notification.create("msg", json['room'], id, list(groupe['id_utilisateurs']))
